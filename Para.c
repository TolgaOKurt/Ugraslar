...
int yenicalc(int* diz,int si){//Max kaça gittiðini buluyor
	int max=0;
	int kontroller=1;
	int buturmu=0;//eger bu tur sayi bulamadýysa bitmiþtir.
	int i=0,j=0;
	for(;1;){
	
		buturmu=0;

		for(i=0;i<=si;i++){//sayi var mý bakýþ
			if(diz[i]==max+1){
				max++;
				kontroller=1;
				buturmu=1;
		...
		for(i=0;i<=si;i++){//toplamda veya farkda sayi var mý bakýþ
			for(j=0;j<=si;j++){//toplamda veya farkda sayi var mý bakýþ				
		...
		if(kontroller==0&&buturmu==0) return max;
	}	
...
void yis(int p){//ana is bölümü
	...
	*/
	...
	for(i=0;1;i++){
		...
		if(test[p-1]<((max/2)-1)) {//pass0   deneysel sonuç/max(son) sayi küçük eþittir 2'ye. eðer deneysel sonuç > 2*max ise. deneysel sonuç nasýl elde edilecek? bir dizide en yüksek sonuç max+max=2*maxtir.
			pas++;
			p0++;				
			continue;
		}
		if(test[(p-1)/2]>max/2){//pass1 dizideki ortanca sayi, tek sayilarda aþaðý yuvarlanacak þekilde, sonucun yarisindan küçük olmali incelendiðinde böyle bir sonuca varýlmýþtýr.!! Kesinlik yok ancak potansiyel var.
			pas++;
			p1++;
			continue;	
		}
